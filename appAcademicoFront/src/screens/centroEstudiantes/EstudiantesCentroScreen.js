...
export default function EstudiantesCentroScreen({ navigation }) {
  ...
  const loadEstudiantes = useCallback(async () => {
    try {
      ...
      const response = await api.get('/usuarios', {
        params: carreraId ? { carrera_id: carreraId } : {},
      });
  ...
  const toggleRolCentro = async (estudiante) => {
    try {
      ...
      const tieneCentro = tieneRolCentro(estudiante);
      let nuevosRoles;
      
      if (tieneCentro) {
        // Quitar rol centro_estudiantes
        nuevosRoles = rolesActuales.filter((r) => r !== 'centro_estudiantes');
      } else {
        // Agregar rol centro_estudiantes
  ...
  return (
    <SafeAreaView style={s.screen}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.headerSub}>
            Asigna el rol de centro estudiantil a estudiantes de tu carrera
          </Text>
        </View>
        ...
        {loading ? (
          ...
          <View style={s.empty}>
            ...
            <Text style={s.emptyText}>
              {busqueda ? 'No se encontraron resultados' : 'No hay estudiantes en tu carrera'}
            </Text>
          </View>
        ) : (
          <View style={s.list}>
              ...
              return (
                <View key={est.id} style={s.card}>
                  <View style={s.cardLeft}>
                    <View style={[s.avatar, { backgroundColor: esCentro ? '#ddd6fe' : '#e2e8f0' }]}>
                      <Text style={[s.avatarText, { color: esCentro ? '#7c3aed' : '#64748b' }]}>
                    ...
                    <View style={s.cardInfo}>
                      ...
                      {esCentro && (
                        <View style={s.badge}>
                          ...
                          <Text style={s.badgeText}>Centro Estudiantil</Text>
                        </View>
                      )}
                    </View>
                  </View>
      ...
      <CentroEstudiantesBottomNav navigation={navigation} active="EstudiantesCentro" />
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  email: { fontSize: 12, color: '#64748b', marginBottom: 6 },
  badge: {
    flexDirection: 'row',
    alignItems: 'center',
